...
class MyObject {
    ...
    // 至于为什么呢，因为你在做项目时，内存的合理使用是很重要的，一个变量声明了static 之后，无乱多少个方法调用它，它在内存中只占用了
    // 一个内存，而没有声明，一个方法调用一次，它就占用一次内存。
...
// 类、对象、方法之间的关系，打个比方，我有个汽车的图纸相当于类，说明汽车要有车身、轮子、方向盘（非静态属性），有个驾驶的
// 功能（非静态方法），你想驾驶汽车。就要根据图纸生产（构建，new）1辆实物汽车（对象\实例），这样你才能实现这个汽车的驾驶功能
// （调用这个实物汽车的方法），如果给这个汽车的图纸起个品牌名字，和那些实例汽车没什么关系，品牌就是静态的，要有个方法来描述这个品牌，
// 这个方法就要声明为静态的，如果在这个图纸里还有个生产摩托车的方法，和车身、轮子、方向盘没什么关系，那这个方法也可以声明为静态的。
